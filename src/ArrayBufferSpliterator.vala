...
namespace Gpseq {
	/**
	 * A spliterator of an array buffer.
	 */
	internal class ArrayBufferSpliterator<G> : Object, Spliterator<G> {
		private ArrayBuffer<G> _array;
		private int _index; // current index
		private int _stop; // zero-based index after the end

		/**
		 * Creates a new array buffer spliterator.
		 * @param array an array buffer
		 * @param start zero-based index of the begin
		 * @param stop zero-based index after the end
		 */
		public ArrayBufferSpliterator (ArrayBuffer<G> array, int start, int stop) {
			_array = array;
			_index = start - 1;
			_stop = stop;
		}
		...
		public Spliterator<G>? try_split () {
			...
			if (_index + 1 >= mid) {
				...
				ArrayBufferSpliterator<G> result = new ArrayBufferSpliterator<G>(_array, _index + 1, mid);
				_index = mid - 1;
				return result;
			}
		}
