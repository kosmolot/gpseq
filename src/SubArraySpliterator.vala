...
namespace Gpseq {
	 ...
	 */
	public class SubArraySpliterator<G> : Object, Spliterator<G> {
		 ...
		 */
		public SubArraySpliterator (SubArray<G> array, int start, int stop) {
			_array = array;
			_index = start - 1;
			_stop = stop;
		}
		...
		public Spliterator<G>? try_split () {
			...
			if (_index + 1 >= mid) {
				...
				SubArraySpliterator<G> result = new SubArraySpliterator<G>(_array, _index + 1, mid);
				_index = mid - 1;
				return result;
			}
		}
