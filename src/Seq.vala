...
namespace Gpseq {
	 ...
	 *
	 * If any operation methods is tried after the seq has been closed, the try
	 * is (assertion) failed.
	 */
	public class Seq<G> : Object {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_generic_array<G> (GenericArray<G> array) {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_iterator<G> (Iterator<G> iterator, int estimated_size, bool size_known) {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_collection<G> (Collection<G> collection) {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_list<G> (Gee.List<G> list) {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_supplier<G> (Supplier<G> supplier) {
		 ...
		 * @return the result seq
		 */
		public static Seq<G> of_supply_func<G> (owned SupplyFunc<G> func) {
			return of_supplier<G>( Supplier.from_func<G>((owned) func) );
		}
		...
		/**
		 * Creates a new sequential seq.
		 *
		 * The seq can be changed to parallel mode by {@link Seq.parallel}.
		 *
		 * @param spliterator a spliterator used by the seq
		 */
		public Seq (Spliterator<G> spliterator) {
			_container = new DefaultContainer<G>(spliterator, null, new Consumer<G>());
			_task_env = TaskEnv.get_default_task_env();
		}

		/**
		 * Creates a new sequential seq with a task env.
		 *
		 * The seq can be changed to parallel mode by {@link Seq.parallel}.
		 *
		 * @param spliterator a spliterator used by the seq
		 * @param task_env task environment
		 */
		public Seq.with_env (Spliterator<G> spliterator, TaskEnv task_env) {
			_container = new DefaultContainer<G>(spliterator, null, new Consumer<G>());
			_task_env = task_env;
		}
		...
		public int count () {
			...
			if (_container.is_size_known && _container.estimated_size >= 0) {
				int result = _container.estimated_size;
				close();
				return result;
			} else if (_is_parallel) {
				return map<int>((g) => { return 1; })
					.fold<int>((g, a) => { return g + a; }, (a, b) => { return a + b; }, 0);
			} else {
		 ...
		 * @return the new seq
		 */
		public Seq<G> distinct (owned HashDataFunc<G>? hash = null,
		...
		public bool all_match (Predicate<G> pred) {
			...
			_container.start(this);
			if (_is_parallel) {
				int len = _container.estimated_size;
				int threshold = _task_env.resolve_threshold(len, _task_env.executor.parallels);
		 ...
		 *
		 * In parallel execution, this operation doesn't respect encounter
		 * order.
		 *
		 * This is a stateful intermediate operation.
		 *
		 ...
		 * @return the new seq
		 */
		public Seq<G> chop (int offset, int length = -1)
			requires (offset >= 0)
		{
			assert(_is_closed == false);
		 ...
		 * @return the new seq
		 */
		public Seq<G> skip_ordered (int n)
			requires (n >= 0)
		{
		 ...
		 * @return the new seq
		 */
		public Seq<G> limit_ordered (int n)
			requires (n >= 0)
		{
		 ...
		 * @return the new seq
		 */
		public Seq<G> chop_ordered (int offset, int length = -1)
			requires (offset >= 0)
		{
			assert(_is_closed == false);
			if (offset == 0 && length < 0) {
		...
		public void foreach (owned Func<G> f) {
			...
			if (_is_parallel) {
				...
				int threshold = _task_env.resolve_threshold(len, _task_env.executor.parallels);
				int max_depth = _task_env.resolve_max_depth(len, _task_env.executor.parallels);
				ForEachTask<G> task = new ForEachTask<G>(_container, f, threshold, max_depth, _task_env.executor);
				task.fork();
				task.join_quietly();
			} else {
				_container.each(f);
		 ...
		 * @see Collector
		 */
		public R collect<A,R> (Collector<G,A,R> collector) {
			assert(_is_closed == false);
			if (_is_parallel) {
		 ...
		 * @see Collector
		 */
		public R collect_ordered<A,R> (Collector<G,A,R> collector) {
			assert(_is_closed == false);
			if (_is_parallel) {
				if (CollectorFeatures.CONCURRENT in collector.features
