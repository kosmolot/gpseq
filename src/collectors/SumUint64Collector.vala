...
private class Gpseq.Collectors.SumUint64Collector<G> : Object, Collector<G,Accumulator,uint64?> {
	private MapFunc<uint64?,G> _mapper;

	public SumUint64Collector (owned MapFunc<uint64?,G> mapper) {
	...
	public uint64? finish (Accumulator a) {
		return a.val;
	}

	public class Accumulator : Object {
		public uint64 val { get; set; }
		public Accumulator (uint64 val) {
			_val = val;
		}
	}
}
