...
public class UtilsTests : Gpseq.TestSuite {
	private const int MANY_SORT_LENGTH = 65536;
	...
	private void test_parallel_sort_stable () {
		var array = new GenericArray<ValueContainer<int>>(MANY_SORT_LENGTH * 2);
		var validation = new GenericArray<ValueContainer<int>>(MANY_SORT_LENGTH * 2);
		for (int i = 0; i < MANY_SORT_LENGTH; i++) {
			var obj = new ValueContainer<int>(0);
			array.add(obj);
			validation.add(obj);
			obj = new ValueContainer<int>((int) Random.next_int());
			array.add(obj);
			validation.add(obj);
		}
		CompareDataFunc<ValueContainer<int>> cmp = (a, b) => {
			int v0 = a.value;
			int v1 = b.value;
			return v0 < v1 ? -1 : (v0 == v1 ? 0 : 1);
		};
		parallel_sort<ValueContainer<int>>(array.data, (a, b) => cmp(a, b));
		// g_ptr_array_sort_with_data is guaranteed to be a stable sort since glib 2.32
		validation.sort_with_data((a, b) => cmp(a, b));
		assert_equal_array<ValueContainer<int>>(array.data, validation.data, (a, b) => a == b);
	}
}
