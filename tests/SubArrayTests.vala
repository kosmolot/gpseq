...
public class SubArrayTests : Gpseq.TestSuite {
	...
	public SubArrayTests () {
		base("sub-array");
		add_test("constructor;data;get;get_owned;size", test_constructor);
	...
	private void test_constructor () {
		...
		assert(array.length == sub.data.length && sub.data.length == sub2.data.length);
		for (int i = 0; i < array.length; i++) {
			assert(array[i] == sub[i] && sub[i] == sub2[i]);
			assert(array[i] == sub.data[i] && sub.data[i] == sub2.data[i]);
	...
	private void test_sort () {
		...
		assert_sorted<int>(sub.data, (a, b) => cmp(a, b));
	}
	...
	private void test_copy () {
		...
		sub2.copy(0, sub, 0, 5);
		assert_equal_array<string>(array, array2);
		assert_equal_array<string>(sub.data, sub2.data);
	}
	...
	private void test_copy_array () {
		...
		assert_equal_array<string>(array, array2);
		assert_equal_array<string>(sub.data, array2);
	}
